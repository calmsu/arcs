...
arcs.views.Resource = (function() {
  __extends(Resource, Backbone.View);
  function Resource() {
  ...
  Resource.prototype.initialize = function() {
    ...
    arcs.bind('resourceChange', __bind(function() {
      arcs.utils.hash.set(this.index + 1);
      return this.render();
    }, this));
  ...
  Resource.prototype.setupCarousel = function(index) {
    $('#carousel').elastislide({
      imageW: 100,
      onClick: __bind(function($item) {
        var id;
        id = $item.find('img').attr('data-id');
        return this.setResourceById(id);
      }, this)
    });
  ...
  Resource.prototype.firstReq = function() {
    if (this.model.get('mime_type') === 'application/pdf') {
      return arcs.utils.modal({
        ...
        buttons: {
          yes: __bind(function() {
            return $.get(arcs.baseURL + 'resources/pdfSplit/' + this.model.id);
          }, this)
        }
      });
    }
